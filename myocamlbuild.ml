...
let pkg_config_lib ~lib (*~has_lib ~stublib *) =
  let cflags = (A"-I") :: (A "../../lablqt/lablqml") :: pkg_config "cflags" lib in (*
...
let files_in_dir path =
  ...
  try
    while true do
      ...
      print_endline s;
      if s<>"." && s<>".." then ans := (path ^/ s) :: !ans
    done;
    []
  with End_of_file -> Unix.closedir d; !ans
...
let () =
  ...
  | After_rules ->
    ...
    flag ["compile"; "ocaml" ] (S[A"-I";A"../../ocaml-pgn/_build"]);
    flag ["compile"; "ocaml" ] (S[A"-I";A"../../lablqt/lablqml/_build"]);

    dep ["link"; "ocaml"; "use_qrc_stub"] ["src/qrc_resources.o"];

    flag ["link"; "ocaml"; "native"; "use_cppstubs" ] (S[A"-linkpkg"; A"src/libcppstubs.a" (*;A"QmlContext.cmx";A"-I";A"../../ocaml-pgn/_build";A"pgn.cmxa"*) ]);
